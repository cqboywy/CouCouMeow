...
export function GrowthRecord({ summary, onStartLearning }: { summary: GrowthSummary; onStartLearning: () => void }) {
  ...
  return <section className="growth-record" aria-labelledby="growth-title">
    ...
    {summary.reviewItems.length > 0 && <Surface className="growth-review"><p className="eyebrow"><Sparkles size={18}/> TODAY'S LITTLE REVIEW</p><h3>今天再抱抱这些小难题</h3><p>刚才没有答对也没关系，再试一次就会更熟啦。</p><div>{summary.reviewItems.map(item => <span key={item.id}><strong lang="en">{item.english}</strong><small>{item.chinese}</small></span>)}</div></Surface>}
